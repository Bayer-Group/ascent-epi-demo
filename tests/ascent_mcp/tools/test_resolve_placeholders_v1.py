"""Unit tests for the v1 ``resolve_placeholders`` tool.

Covers:
- placeholder-pattern routing: the 2- and 3-segment patterns must be
  disjoint so each placeholder hits exactly one resolver
- non-OMOP (3-segment) path: vocabulary explicit, no database required
- OMOP (2-segment) path: vocabulary implied by domain, database required
- mixed SQL: both formats in one query, each routed correctly
- ``list_placeholder_concepts``: reports both formats
"""
...
# ``tools_v1`` imports ``ascent_domain.databases``, which pulls in
# ``ascent_domain.models.database`` <-> ``data_definitions`` (a circular
# import only triggered when nothing else in the process has loaded the
# bottom of the cycle yet — production startup orders things so it works).
# Force-load ``data_definitions`` first so importing ``tools_v1`` below
# resolves both sides of the cycle.
import ascent_domain.models.data_definitions  # noqa: F401, E402
from ascent_mcp.tools_v1 import (  # noqa: E402
...
def test_omop_pattern_matches_two_only():
    ...
    # The 2nd '@' before ']' keeps a 3-segment placeholder out of the OMOP matcher.
...
def test_two_adjacent_omop_do_not_bridge_into_false_three_segment():
    # Regression: a naive ``[^@]+`` middle group greedily spans the
    # "]) ... ([" gap between two OMOP placeholders, yielding a bogus 3-segment
    # match and misrouting genuine OMOP SQL into the non-OMOP path.
...
async def test_three_segment_resolved_with_concept_ids():
    ...
    with patch(
        ...
        AsyncMock(return_value=fake_entities),
    ):
...
async def test_non_omop_forwards_database_to_coder():
    ...
    coder = AsyncMock(return_value=fake_entities)
    with patch("ascent_mcp._tools_shared.get_medical_concepts_from_sql_async", coder):
...
async def test_omop_coding_system_source_forwarded_to_post_process_query():
    """``coding_system='Source'`` must reach ``post_process_query`` as ``SOURCE_CODING``."""
...
async def test_omop_concept_not_found_reports_warning_and_keeps_sql():
    ...
    # On failure the original SQL is preserved and the failure is reported.
    assert result["resolved_sql"] == sql
